...
class Recognizer(AudioSource):
    ...
    def listen(
        ...
        """
        result = self._listen(
            source, timeout, phrase_time_limit, snowboy_configuration, stream, openwakeword_model
        )
        if not stream:
            for a in result:
                return a
    ...
    def _listen(
        ...
        buffer = (
            b""  # an empty buffer means that the stream has ended and there is no data left to read
        )
        while True:
    ...
    def recognize_azure(
        ...
        access_token, expire_time = getattr(self, "azure_cached_access_token", None), getattr(
            self, "azure_cached_access_token_expiry", None
        )
        allow_caching = True
        try:
            from time import (
        ...
        url = (
            ...
            + ".stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1?{}".format(
                urlencode({"language": language, "format": result_format, "profanity": profanity})
            )
        )
        ...
        if check_existing:
            ...
            try:
                ...
                if exc.response["Error"][
                    "Code"
                ] == "BadRequestException" and "The requested job couldn't be found" in str(exc):
    ...
    def recognize_assemblyai(self, audio_data, api_token, job_name=None, **kwargs):
        ...
        if check_existing:
            ...
            response = requests.post(
                "https://api.assemblyai.com/v2/upload", headers=headers, data=read_file(audio_data)
            )
